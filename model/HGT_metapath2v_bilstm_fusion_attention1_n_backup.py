...
class MultiHeadCrossAttentionByCFG(nn.Module):
    ...
    def forward(self, hgt_emb, sem_emb, context_matrix, context_mask):
        N, D = hgt_emb.shape
        H, d_h = self.num_heads, self.head_dim

        Q = self.q_proj(hgt_emb).view(N, H, d_h)
        K_all = self.k_proj(sem_emb).view(N, H, d_h)
        V_all = self.v_proj(sem_emb).view(N, H, d_h)

        K_ctx = K_all[context_matrix]  # [N, L, H, d_h]
        V_ctx = V_all[context_matrix]

        Q = Q.unsqueeze(2)  # [N, H, 1, d_h]
        attn_scores = torch.einsum("nhqd,nlhd->nhql", Q, K_ctx).squeeze(2) / self.scale  # [N, H, L]

        attn_scores = attn_scores.masked_fill(~context_mask.unsqueeze(1), float('-inf'))
        attn_weights = torch.softmax(attn_scores, dim=-1)  # [N, H, L]
        out = torch.einsum("nhl,nlhd->nhd", attn_weights, V_ctx)  # [N, H, d_h]

        out_cat = out.contiguous().view(N, D)
        return self.out_proj(out_cat)
...
class HGTVulNodeClassifier(nn.Module):
    def __init__(self, compressed_global_graph_path, semantic_embed_path, feature_extractor=None, node_feature='metapath2vec', hidden_size=128, num_layers=2,num_heads=8, use_norm=True, device='cpu',precomputed_feature_path=None):
        ...
        #context_dict = build_context_by_metapath_type(self.symmetrical_global_graph, self.meta_paths)
        #self.context_matrix, self.context_mask = build_context_tensor(context_dict, num_nodes, max_ctx_len=64, device=device)

        context_dict = build_context_by_metapath_type(
            graph=nx_graph,
            meta_paths=self.meta_paths, 
            max_ctx_len=64
        )
        self.context_matrix, self.context_mask = build_context_tensor(
            node_to_context=context_dict,
            num_nodes=self.symmetrical_global_graph.number_of_nodes(),
            max_ctx_len=64,
            device=self.device
        )
        context_lens = [len(v) for v in context_dict.values()]
        #print(f"Max length: {max(context_lens)}")
        #print(f">64 context: {(sum(l > 64 for l in context_lens))} nodes")
        
        count_duplicate_context_rows(self.context_matrix)
        count_self_only_context(self.context_matrix)
        ...
        self.cross_attention = MultiHeadCrossAttentionByCFG(input_dim=128, num_heads=4)
        self.layernorm = nn.LayerNorm(hidden_size)
        self.bilstm = nn.LSTM(self.hidden_size + self.hidden_size , self.hidden_size,batch_first=True, bidirectional=True) 
    ...
    def forward(self, node_ids=None):
        ...
        for ntype, feature in h.items():
            ...
            hiddens[self.node_ids_dict[ntype]] = feature  # gộp về dạng tensor chung

        sem = self.raw_sem_feats
        sem = self.semantic_proj(sem)  # [N, D]

        # === Cross attention fusion
        attn_out = self.cross_attention(hiddens, sem, self.context_matrix, self.context_mask)  # [N, D]
        attn_out = self.dropout(self.layernorm(attn_out))
        #fused = attn_out
        #fused = hiddens + attn_out
        fused = torch.cat([hiddens, attn_out], dim=1)  # [N, 2D]

        #print(f"[FORWARD] fused shape: {fused.shape}", flush=True)
        #print(f"fused : {fused}", flush=True)

        output, _ = self.bilstm(fused.unsqueeze(0))  # [1, N, 2D]
        output = self.dropout(output)
        output = self.classify(output)  # [1, N, C]
        return output  # [N, C]
...
def build_context_by_metapath_type(graph, meta_paths, max_ctx_len=64):
    node_to_context = defaultdict(set)

    for metapath in meta_paths:
        for start_node, data in graph.nodes(data=True):
            if data.get("node_type") != metapath[0][0]:
                continue
            ...
            for src_type, edge_type, dst_type in metapath:
                ...
                for node in current_nodes:
                    for succ in graph.successors(node):
                        edge_data = list(graph[node][succ].values())[0]
                        if edge_data.get("label") == edge_type and graph.nodes[succ].get("node_type") == dst_type:
                            next_nodes.append(succ)
                current_nodes = next_nodes
                if not current_nodes:
                    break

            for ctx_node in current_nodes:
                node_to_context[start_node].add(ctx_node)

    final_dict = {}
    for k, v in node_to_context.items():
        ctx = list(v)[:max_ctx_len]
        if not ctx:
            ctx = [k]
        final_dict[k] = ctx
...
def build_context_tensor(node_to_context, num_nodes, max_ctx_len, device):
    context_matrix = torch.full((num_nodes, max_ctx_len), -1, dtype=torch.long, device=device)
    context_mask = torch.zeros((num_nodes, max_ctx_len), dtype=torch.bool, device=device)

    for i in range(num_nodes):
        ctx = node_to_context.get(i, [])
        if len(ctx) == 0:
            ctx = [i]  # fallback: chính nó
        ctx = ctx[:max_ctx_len]
        context_matrix[i, :len(ctx)] = torch.tensor(ctx, dtype=torch.long, device=device)
        context_mask[i, :len(ctx)] = True

    return context_matrix, context_mask

def count_duplicate_context_rows(context_matrix):
    """
    Đếm số dòng context_matrix bị trùng nhau hoàn toàn.
    """
    # Đưa về dạng list các tuple để dễ hash
    row_tuples = [tuple(row.tolist()) for row in context_matrix.cpu()]
    
    # Dùng set để tìm số lượng unique
    unique_rows = set(row_tuples)
    
    num_total = len(row_tuples)
    num_unique = len(unique_rows)
    num_duplicates = num_total - num_unique
    
    print(f"Tổng node: {num_total}")
    print(f"Số dòng context unique: {num_unique}")
    print(f"Số dòng context bị trùng: {num_duplicates} ({(num_duplicates / num_total) * 100:.2f}%)")

# Gọi hàm


def count_self_only_context(context_matrix):
    """
    Đếm số dòng chỉ attend chính nó (node i → [i])
    """
    identity_context = torch.arange(context_matrix.size(0)).unsqueeze(1).to(context_matrix.device)  # [N, 1]
    self_only = (context_matrix == identity_context).all(dim=1)  # [N] boolean
    
    num_self_only = self_only.sum().item()
    print(f"Số node chỉ attend chính nó: {num_self_only} ({(num_self_only / context_matrix.size(0)) * 100:.2f}%)")
